...
export class PhaseFormComponent implements OnInit {
  ...
  onKey(event) {
    this.cleanTextToSay.forEach((element, index) => {
      if (this.cleanTextToSay[index].includes(event.target.name)) {
        ...
        console.log(
          this.cleanTextToSay[index].replace(
            event.target.name,
            event.target.value
          )
        );
  ...
  ngOnInit() {
    this.textToSay = this.textToSayConverterService.getTextWithoutParameters(
      this.filteredPhase.texttosay,
      this.phaseInputs
    );
    this.textToSayString = this.textToSay.join();
  }
  ...
  ngOnChanges() {
    ...
    if (this.selectedPhaseDetail) {
      this.filteredPhase = this.phases.find((phase) => {
        if (phase.phasedetail == this.selectedPhaseDetail) return phase;
      });
